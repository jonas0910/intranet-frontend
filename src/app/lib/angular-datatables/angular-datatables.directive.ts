...
export class DataTableDirective implements OnDestroy, OnInit {
    ...
    this.dtInstance = new Promise((resolve, reject) => {
      Promise.resolve(this.dtOptions).then(resolvedDTOptions => {
        ...
        if (resolvedDTOptions['columns']) {
          resolvedDTOptions['columns'].forEach((col: any) => {
            if ((col.id ?? '').trim() === '') {
  ...
  private applyNgPipeTransform(row: Node, columns: ADTColumns[]): void {
    ...
    const colsWithPipe = columns.filter((x: any) => x['ngPipeInstance'] && !x['ngTemplateRef']);
    colsWithPipe.forEach((el: any) => {
      const pipe = el['ngPipeInstance']!;
      const pipeArgs = el['ngPipeArgs'] || [];
      // find index of column using `data` attr
      const i = columns.filter((c: any) => c['visible'] !== false).findIndex((e: any) => e['id'] === el['id']);
  ...
  private applyNgRefTemplate(row: Node, columns: ADTColumns[], data: Object): void {
    ...
    const colsWithTemplate = columns.filter((x: any) => x['ngTemplateRef'] && !x['ngPipeInstance']);
    colsWithTemplate.forEach((el: any) => {
