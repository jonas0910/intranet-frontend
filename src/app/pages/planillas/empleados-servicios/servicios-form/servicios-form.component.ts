...
import { ServiciosContractModalComponent } from '../servicios-contract-modal/servicios-contract-modal.component';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, ServiciosContractModalComponent],
  templateUrl: './servicios-form.component.html',
  styleUrls: ['./servicios-form.component.scss']
})
export class ServiciosFormComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['employee'] && changes['employee'].currentValue) {
      ...
      console.log('🔄 Employee input changed, updating state');
      
      this.handleModalMode();
      this.checkRestrictions(); // Asegurarse de re-aplicar restricciones
      
      // Cargar contratos si está en modo tabs y es edición
      if (this.useTabs && this.isEditMode && this.employeeId) {
  ...
  private initializeForm(): void {
    this.employeeForm = this.fb.group({
      ...
      position_id: [''],
      cost_center_id: [''],
      labor_regime: ['servicio', [Validators.required]],
  ...
  private populateForm(employee: Employee): void {
    ...
    this.employeeForm.patchValue({
      ...
      labor_regime: employee.labor_regime || 'servicio',
      ...
      receives_family_allowance: !!employee.receives_family_allowance,
      is_unionized: !!employee.is_unionized,
      has_life_insurance: !!employee.has_life_insurance
    });
  }
      ...
      next: (response) => {
        ...
        if (response.success) {
          ...
          if (this.isModal) {
            ...
            // Intentar regresar a la página anterior en lugar de una fija
            if (window.location.pathname.includes('/empleados-servicios')) {
               this.router.navigate(['/planillas/empleados-servicios']);
            } else {
               this.router.navigate(['/planillas/empleados']);
            }
          }
   ...
   */
  getPageTitle(): string {
    return this.isEditMode ? 'Editar Personal de Servicio' : 'Nuevo Personal de Servicio';
  }
        ...
        try {
          this.contractsDataTable = $('#contractsTable').DataTable({
            ...
            language: {
              ...
              search: 'Buscar:',
              lengthMenu: `Mostrar _MENU_ ${this.isModal ? 'órdenes' : 'contratos'}`,
              info: `Mostrando _START_ a _END_ de _TOTAL_ ${this.isModal ? 'órdenes' : 'contratos'}`,
              infoEmpty: `No hay ${this.isModal ? 'órdenes' : 'contratos'}`,
              infoFiltered: `(filtrado de _MAX_ ${this.isModal ? 'órdenes' : 'contratos'} totales)`,
              zeroRecords: `No se encontraron ${this.isModal ? 'órdenes' : 'contratos'}`,
              emptyTable: `No hay ${this.isModal ? 'órdenes' : 'contratos'} disponibles`,
            ...
            columnDefs: [
              { orderable: false, targets: 7 } // Columna de acciones (índice 7 para 8 columnas)
            ]
          });
  ...
  toggleContractActions(contractId: any, event: Event): void {
    event.stopPropagation();
    console.log('🔘 Toggle acciones para ID:', contractId);
    if (this.openActionsContractId === contractId) {
      this.openActionsContractId = null;
    } else {
      this.openActionsContractId = contractId;
    }
  }
  ...
  private checkRestrictions(): void {
    ...
    // Si estamos en modo modal (desde el módulo de Servicios)
    if (this.isModal) {
      // Forzar régimen de servicio y limpiar validadores si necesario
      this.employeeForm.get('labor_regime')?.patchValue('servicio');
      this.employeeForm.get('labor_regime')?.clearValidators();
      this.employeeForm.get('labor_regime')?.updateValueAndValidity();
      this.employeeForm.get('labor_regime')?.disable();
      
      // La previsional no va para servicios, así que quitamos obligatoriedad
      this.employeeForm.get('pension_system')?.clearValidators();
      this.employeeForm.get('pension_system')?.updateValueAndValidity();

      // Forzar departamento del usuario si no es super admin
      if (user && user.empleado) {
        const dept = (user.empleado as any).departamento || (user.empleado as any).department;
        const deptId = dept?.id || (user.empleado as any).department_id || (user.empleado as any).departamento_id;
        
        if (deptId) {
          this.employeeForm.get('department_id')?.patchValue(deptId);
          this.employeeForm.get('department_id')?.disable();
          console.log('🏢 Departamento asignado en formulario:', deptId);
        }
      }
    } else if (!this.employee && !this.isEditMode) {
      // Lógica original para modo página normal (si se necesita)
      if (user && user.empleado) {
        const dept = (user.empleado as any).departamento || (user.empleado as any).department;
        const deptId = dept?.id || (user.empleado as any).department_id || (user.empleado as any).departamento_id;
