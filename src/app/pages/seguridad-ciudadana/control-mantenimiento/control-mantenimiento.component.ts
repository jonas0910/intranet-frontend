...
@Component({
  selector: 'app-sc-control-mantenimiento',
  standalone: true,
  ...
  styleUrl: './control-mantenimiento.component.scss'
})
export class ScControlMantenimientoComponent implements OnInit, AfterViewInit {
  controles: any[] = [];
  alertas: any[] = [];
  ...
  initDataTable(): void {
    ...
    if ($.fn.DataTable.isDataTable('#tablaScControles')) {
      $('#tablaScControles').DataTable().destroy();
    }
    this.dataTable = $('#tablaScControles').DataTable({
  ...
  marcarRealizado(control: any, content?: any): void {
    ...
    if (content) {
      ...
      if (confirm(`¿Desea registrar un nuevo mantenimiento para atender esta alerta?`)) {
        this.router.navigate(['/seguridad-ciudadana/mantenimientos'], {
  ...
  private ejecutarReprogramar(control: any, nuevaFecha: string, motivo: string): void {
    const datos = {
        fecha_proxima: nuevaFecha, kilometraje_proximo: this.nuevoKilometraje, horometro_proximo: this.nuevoHorometro,
  ...
  generarControlesEquipo(equipoId: number): void {
    if (!confirm('¿Generar controles obligatorios para este vehículo?')) return;
